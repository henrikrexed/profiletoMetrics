...
func NewProfilesToMetricsConnector(config ConverterConfig, nextConsumer consumer.Metrics) *ProfilesToMetricsConnector {
...
func (c *ProfilesToMetricsConnector) ConsumeProfiles(ctx context.Context, profiles pprofile.Profiles) error {
	...
	converter := NewConverterConnector(c.config)
